...
import { Plus, DownloadSimple, List, X, House, Trash, Gear, ArrowLeft, ToggleLeft } from '@phosphor-icons/react'
...
function App() {
  ...
  const [newSettingType, setNewSettingType] = useState<'toggle' | 'input' | 'select'>('toggle')
    ...
    setConfig((current) => {
      ...
      return {
        ...
        components: {
          ...
          settings: {
            ...base.components.settings,
            items: base.components.settings.items.filter(item => item.id !== id)
          }
        }
      }
    })
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card className="p-4">
          ...
          <ScrollArea className="w-full">
            <div className="flex gap-2 pb-2">
              ...
              <Button
                variant={safeConfig.components.settings.shown ? "default" : "outline"}
                className={safeConfig.components.settings.shown ? "bg-accent text-accent-foreground hover:bg-accent/90" : ""}
        ...
        <AnimatePresence>
          {safeConfig.components.settings.shown && (
            ...
            >
              <Card className="p-4">
                <h2 className="text-lg font-semibold mb-4 font-mono">Settings Items</h2>
                ...
                <div className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={newSettingType} onValueChange={(value: 'toggle' | 'input' | 'select') => setNewSettingType(value)}>
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        {safeConfig.components.settings.items.map((item) => (
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              <ToggleLeft size={20} className="text-muted-foreground flex-shrink-0" />
                              <div className="min-w-0 flex-1">
                                <p className="font-medium truncate">{item.label}</p>
                                <p className="text-sm text-muted-foreground truncate capitalize">{item.type}</p>
                              </div>
                            </div>
                            <Button
                              variant="ghost"
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {(safeConfig.components.navigationDrawer.shown || safeConfig.components.settings.shown) && (
          ...
          <div className="relative border-2 border-border rounded-lg overflow-hidden bg-card min-h-[400px]">
            ...
            {previewPage === 'settings' && safeConfig.components.settings.shown && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
